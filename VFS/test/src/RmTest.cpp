...
TEST_F(RmTest, WhenRmIsCalledAndFileExists_ThenItReturnsFileRemoved)
{
    ...
    EXPECT_CALL(*mockDirectory, removeComponent(std::dynamic_pointer_cast<IFileSystemComponent>(mockFile)));
...
TEST_F(RmTest, WhenRmIsCalledToDeleteDirectory_ItReturnsCannotDeleteADirectory)
{
    ...
    EXPECT_CALL(*testDirectory, getType()).WillOnce(::testing::Return("Directory"));
    std::string expectedResponse = "test_dir is not a file.\n";
    EXPECT_EQ(rm->execute(*mockFileSystem), expectedResponse);
}